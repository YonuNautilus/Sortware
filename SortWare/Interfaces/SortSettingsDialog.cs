...
namespace SortWare
{
    ...
    public partial class SortSettingsDialog
    {
        ...
        public void refreshTagsViewer()
        {
            ...
            if (SettingsTreeView.SelectedNode.Tag is not null && SettingsTreeView.SelectedNode.Tag is SortDirectory && ((SortDirectory)SettingsTreeView.SelectedNode.Tag).type == SortSettings.dirType.MAINDIR)
            {
                if (((SortDirectory)SettingsTreeView.SelectedNode.Tag).hasTags())
                {
                    string[] tagArr = ((SortDirectory)SettingsTreeView.SelectedNode.Tag).getTags();
                    for (int t = 0, loopTo = tagArr.Length - 1; t <= loopTo; t++)
    ...
    /// </summary>
        ...
        private void AddTagButton_Click(object sender, EventArgs e)
        {
            if (TagIDEntry.Text is not null && TagIDEntry.Text.Trim()[TagIDEntry.Text.Length - 1] == '.')
            {
                ...
                {
                    TagsViewer.Items.Add((TagIDEntry.Text + Constants.vbTab + TagDescEntry.Text.Trim()).Trim());
                }
            }
        }
        ...
        private void TagsSaveButton_Click(object sender, EventArgs e)
        {
            string o = "";
            foreach (var item in TagsViewer.Items)
                o = o + item.ToString() + Constants.vbNewLine;
            if (SettingsTreeView.SelectedNode.Tag is SortDirectory && ((SortDirectory)SettingsTreeView.SelectedNode.Tag).type == SortSettings.dirType.MAINDIR)
            {
                ((SortDirectory)SettingsTreeView.SelectedNode.Tag).saveTags(o);
            }
        }
