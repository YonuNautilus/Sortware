...
namespace SortWare
{
    public partial class MediaViewer
    {
        ...
        public MediaViewer()
        {
            ...
            InitializeComponent();
            VlcControl1 = _VlcControl1;
            VideoScrollBar = _VideoScrollBar;
            _VlcControl1.Name = "VlcControl1";
    ...
    /// </summary>
        ...
        {
            if (VlcControl1.GetCurrentMedia() is not null && VlcControl1.GetCurrentMedia().Mrl is not null)
            {
                string s = new Uri(VlcControl1.GetCurrentMedia().Mrl).LocalPath;
        ...
        {
            string s = new Uri(VlcControl1.GetCurrentMedia().Mrl).LocalPath;
            if (VlcControl1.IsPlaying && s.Contains(path))
            {
                return true;
            }
            else
        ...
        private void VlcControl1_LengthChanged(object Sender, EventArgs e)
        {
            ...
            VideoScrollBar.Invoke(new Action(() =>
                {
                    VideoScrollBar.Maximum = (int)VlcControl1.Length;
        ...
        private void Timer1_Tick(object sender, EventArgs e) // , Me.VlcMediaPlaying
        {
            ...
            try
            {
                TimeLabel.Invoke(new Action(() =>
                    {
                        float fps = VlcControl1.VlcMediaPlayer.FramesPerSecond;
                        if (fps > 0f)
                        {
                            // TimeLabel.Text = getTimeCode(VlcControl1.Time, fps) & " / " & getTimeCode(VlcControl1.Length, fps)
                            TimeLabel.Text = getTimeCode(VlcControl1.Time, (double)fps) + Constants.vbNewLine + getTimeCode(VlcControl1.Length, (double)fps);
                        }
                    }));
            }
            catch (Exception Ex)
